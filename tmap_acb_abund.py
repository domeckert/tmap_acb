...
def fit_values(all_img, all_exp, fint, cx, cy, ncount, pixsize, skybkg, all_bkg=None, method='cstat', fit_ab=False, countimage=None):
    ...
    profile, eprof, bkgprof = np.zeros(nimg), np.zeros(nimg), np.zeros(nimg)
    ...
    for i in range(nimg):
        ...
        if method=='cstat':
            ...
            if all_bkg is not None:
                bkgprof[i] = np.sum(all_bkg[i][sel] / all_exp[i][sel]) / nv / pixsize ** 2
            else:
                bkgprof[i] = 0.
...
def main(argv):
    try:
        opts, args = getopt.getopt(argv,"i:e:t:o:m:c:b:r:i:s:g:n:",["images=","expmaps=","template=","mask=","outfile=","ncount=","bkgmaps=","rmax=","tmin=","regfile=","skybkg=","countimage="])
    except getopt.GetoptError:
        print('tmap_acb.py -i imglist -e explist -t template -o outfile --bkgmaps=bkglist --mask=mask.fits --ncount=200 --rmax=3 --tmin=0.5 --regfile=file.reg --skybkg=skybkg.dat --countimage=imagelink')
        sys.exit(2)
    ...
    for opt, arg in opts:
        if opt in ("-i", "--images"):
            ...
            fit_skybkg = False
        elif opt in ("-n", "--countimage"):
            ncf = arg
            print('We will use the image provided in',ncf,'as cumulative count image')
            fnc = open(ncf)
            lnc = fnc.readline()
            fnc.close()

            ffnc = fits.open(lnc.split()[0])
            countimage = ffnc[0].data
            ffnc.close()
    ...
    abmap, eabmap = np.zeros(shape), np.zeros(shape)
    ...
    for j in range(shape[0]):
        ...
        for i in range(shape[1]):
            ...
            if mask[j, i] > 0 and rads[j, i]<=rmax:
                ...
                tkt, ekt, norm, enorm, ab, eab, rad = fit_values(all_img=all_img,
                                                          all_exp=all_exp,
                                                          fint=all_fint,
                                                          cx=i,
                                                          cy=j,
